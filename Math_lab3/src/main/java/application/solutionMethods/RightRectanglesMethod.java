...
public class RightRectanglesMethod implements SolutionMethod{
    @Override
        ...
        if (!function.hasBreakingPoint()) {
            ...
            for (int i = 0; i < 100; i++) {
                if (section[0] + step * (i + 1) == function.getBreakingPoint()){
                    double averageValue = (function.getValue(section[0] + step * (i + 1) + eps) + function.getValue(section[0] + step * (i + 1) - eps)) / 2;
                    result += averageValue;
                } else {
                    result += function.getValue(section[0] + step * (i + 1));
                }
